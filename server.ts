...
async function callOpenRouter(messages: any[], model: string = "google/gemma-3-27b-it:free"): Promise<string> {
...
app.post('/api/llm/generate', async (req, res) => {
    try {
        ...
        let outlineRaw = await callOpenRouter(outlineMessages, "google/gemma-3-27b-it:free");
        ...
        const nodePromises = subtopics.map(async (subtopic: string, index: number) => {
            ...
            try {
                let nodeContent = await callOpenRouter(messages, "google/gemma-3-27b-it:free");
